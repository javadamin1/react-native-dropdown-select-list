...
const SelectList: React.FC<SelectListProps> = ({
                                                 ...
                                                 notFoundText = isRtl ? 'دیتا وجود ندارد' : "No data found",
                                                 disabledItemStyles,
                                                 disabledTextStyles,
                                                 onSelect = () => { },
                                                 save = 'key',
  ...
  const body = isRtl ? styles.rtlBody : styles.body
  const wrapper = isRtl ? styles.rtlWrapper : styles.wrapper
  const searchStyle = isRtl ? styles.rtlSearch : styles.search
  const searchInputStyle = isRtl ? styles.rtlSearchInput : styles.searchInput
    ...
    <View>
      {
        (dropdown && search)
          ...
          <View style={[wrapper, boxStyles]}>
            <View style={body}>
              ...
              <TextInput
                ...
                style={[searchInputStyle, { fontFamily }, inputStyles]}
              />
          ...
          :
          <TouchableOpacity style={[wrapper, boxStyles]} onPress={() => { if (!dropdown) { Keyboard.dismiss(); slidedown() } else { slideup() } }}>
            <Text style={[{ fontFamily }, inputStyles]}>{(selectedval == "") ? (placeholder) ? placeholder : isRtl ? 'انتخاب کنید' :'Select option' : selectedval}</Text>
            {
              (!arrowicon)
                ?
                <Image
...
const styles = StyleSheet.create({
  ...
  rtlWrapper: { borderWidth: 1, borderRadius: 10, borderColor: 'gray', paddingHorizontal: 20, paddingVertical: 12, flexDirection: 'row-reverse', justifyContent: 'space-between' },
  ...
  rtlBody: {
    flexDirection: 'row-reverse', alignItems: 'center', flex: 1
  },
  search: {
  ...
  ,
  searchInput: {
    padding: 0, height: 20, flex: 1,textAlign:'left'
  },
  rtlSearchInput: {
    padding: 0, height: 20, flex: 1,textAlign:'right'
  }
